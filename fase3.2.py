...
def buscarProdutos():
    global num_tela, contador,designerTabela,opção_editar# tornando as variaveis global
    print("\n")
    print("_"*55)
...
def cadastroProdutos():
    ...
    tela_cadastro.lower()
    while not tela_cadastro=='sim' or tela_cadastro=='não':
        tela_cadastro = Verificação("Opção Desejada: ",str)
        tela_cadastro.lower()
...
def editarProdutos():
    ...
    if num_tela==3:
        if num_tela==3 and contador==0:
            ...
            while Verificar_Dados not in[1,2]:#Verificação de entrada
                print("\t\033[41mERRO\033[0m\n")
                Verificar_Dados=Verificação("\033[47m\033[30mOpção Desejada: \033[0m",float)

            if Verificar_Dados==1:#Verificar dados ==1 volta para tela de pesquisa do programa
...
def apagarProdutos(codigo_produto):
    ...
    if num_tela==4 and contador>=1:
            ...
            cursor.execute(f"DELETE FROM Proodutos WHERE codigo = {codigo_produto}")
            cursor.execute("commit")
